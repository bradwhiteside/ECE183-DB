...
def init_plot(plt_show):
    ...
    lines.extend(axes[2, 1].plot(dummy_1D_values, dummy_1D_values, label="overshoot max"))

    return fig1, axes, lines


# Plot the path
def plot_results(figure, axes, lines, times, true_states, est_states, torques, speeds, accels, input_goal, overshoots,
                 avg_velocity,
                 plt_pause=False, plt_show=False):
    if not plt_show or figure is None or axes is None or lines is None:
        return
    ...
    lines[i].set_data(times, overshoots[i_1:i_2, 0])
    i += 1
    lines[i].set_data(times, overshoots[i_1:i_2, 1])
    i += 1
...
def plot_all_results(times, true_states, est_states, torques, speeds, accels, input_goal, overshoots, plt_pause=False,
                     plt_show=False):
    if (plt_show == True):
        i_1 = 0
        i_2 = -1
